...
def rule_dict_create(ori_rule,x_rule,**kwargs):
    ...
    if "Interact" in kwargs:
        for interact in kwargs["Interact"]:
...
def dict_id_create_complex_rules(IdDescription,EventName,Level,PointId,Description,DeviceType,Floor,DeviceId,GUID,calculator,**kwargs):
    if ("Message_Title" in kwargs) and ("Message" in kwargs) and ("SilentTime" in kwargs) and ("message_target" in kwargs):
...
def main(EventName,Level,PointId,Description,DeviceType,DeviceId,Floor,GUID,x_rule,y_rule,**kwargs):
    ...
    if ("Message_Title" in kwargs) and ("Message" in kwargs) and ("SilentTime" in kwargs) and ("message_target" in kwargs):
        dict_id = dict_id_create_complex_rules(IdDescription=IdDescription,
        EventName=EventName,
        Level=Level,
        PointId=PointId,
